...
export const styles = css`
  ...
  event-log .log-entry {
    ...
    &[data-type='flow'] {
      --entry-color: var(--sn-node-selected, #4a9eff);
    }
    &[data-type='node'] {
      --entry-color: var(--sn-cat-data, #a78bfa);
    }
    &[data-type='connection'] {
      --entry-color: var(--sn-success-color, #5cd87a);
    }
    &[data-type='success'] {
      --entry-color: var(--sn-success-color, #5cd87a);
    }
    &[data-type='error'] {
      --entry-color: var(--sn-danger-color, #ef4444);
    }
    &[data-type='info'] {
      --entry-color: var(--sn-warning-color, #e2b456);
    }
  }
