...
function createAgentSnapshot(layout, rects, results) {
  ...
  return {
    schema: 'symbiote-node.pcb-stress.v1',
    frame: state.frames,
...
function runOrbitSamples({ samples = 120, includeRoutes = false } = {}) {
  ...
  return {
    schema: 'symbiote-node.pcb-orbit-samples.v1',
    samples: sampleCount,
    diagnosticsOptions,
    aggregate: {
...
function publishAgentDiagnostics(snapshot) {
  ...
  const envelope = {
    schema: 'symbiote-node.pcb-stress-agent-state.v1',
    capabilities: {
  ...
  window.__pcbStressDiagnostics = {
    schema: 'symbiote-node.pcb-stress-api.v1',
    snapshot,
    keyframes: state.keyframes,
...
function writeAgentResult(command, payload, error = null) {
  writeAgentJson(agentResultElementId, {
    schema: 'symbiote-node.pcb-stress-agent-result.v1',
...
function maybeCaptureKeyframe(layout, rects, results) {
  ...
  state.keyframes.unshift({
    schema: 'symbiote-node.pcb-keyframe.v1',
