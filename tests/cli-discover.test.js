...
describe('discover command', () => {
  ...
  describe('manifest', () => {
    ...
    it('exposes experimental renderer capabilities', () => {
      ...
      assert.ok(webxr.capabilities.includes('xr-session-controller'));
      assert.ok(webxr.capabilities.includes('xr-session-options-normalization'));
      assert.ok(webxr.capabilities.includes('xr-controller-input-normalization'));
      assert.ok(webxr.capabilities.includes('xr-theme-bridge'));
      assert.ok(webxr.capabilities.includes('xr-panel-host'));
      assert.ok(webxr.capabilities.includes('xr-dom-panel-workbench'));
      assert.ok(webxr.capabilities.includes('xr-content-pointer-target'));
      assert.ok(webxr.capabilities.includes('xr-texture-quality-diagnostics'));
      assert.ok(webxr.capabilities.includes('xr-scene-quality-diagnostics'));
      assert.ok(webxr.capabilities.includes('xr-pose-comfort-diagnostics'));
      assert.ok(webxr.capabilities.includes('xr-pose-comfort-adjustment'));
      assert.ok(webxr.capabilities.includes('xr-facing-diagnostics'));
      assert.ok(webxr.capabilities.includes('xr-facing-adjustment'));
      assert.ok(webxr.capabilities.includes('xr-panel-gesture'));
      assert.ok(webxr.capabilities.includes('xr-layout-transaction'));
      assert.ok(webxr.capabilities.includes('xr-deep-graph-scene'));
      assert.ok(webxr.capabilities.includes('xr-deep-graph-focus'));
      assert.ok(webxr.capabilities.includes('xr-room-aware-placement-contract'));
      assert.ok(webxr.capabilities.includes('xr-voice-command-contract'));
      assert.ok(webxr.capabilities.includes('xr-scene-diagnostics'));
      assert.ok(webxr.capabilities.includes('xr-readiness-diagnostics'));
      assert.ok(webxr.capabilities.includes('xr-webgl-layer-target'));
      assert.ok(webxr.capabilities.includes('xr-webgl-layer-panel-renderer'));
      ...
      assert.ok(webxr.capabilities.includes('xr-texture-debug-mode'));
      assert.ok(webxr.capabilities.includes('xr-texture-gate-diagnostics'));
      ...
      assert.ok(threeWebxr.capabilities.includes('three-ray-plane-panel-drag'));
      assert.ok(threeWebxr.capabilities.includes('three-controller-ray-visuals'));
      assert.ok(threeWebxr.capabilities.includes('three-panel-hit-reticle'));
      assert.ok(threeWebxr.capabilities.includes('three-interaction-state-diagnostics'));
      assert.ok(threeWebxr.capabilities.includes('three-session-telemetry-snapshot'));
      assert.ok(threeWebxr.capabilities.includes('three-session-health-summary'));
      assert.ok(threeWebxr.capabilities.includes('three-troubleshooting-summary'));
      assert.ok(threeWebxr.capabilities.includes('three-drag-response-filter'));
      assert.ok(threeWebxr.capabilities.includes('three-dom-texture-material-bridge'));
      assert.ok(threeWebxr.capabilities.includes('three-html-canvas-texture-resolver'));
      assert.ok(threeWebxr.capabilities.includes('three-strict-texture-fail-fast'));
      assert.ok(threeWebxr.capabilities.includes('three-session-controller'));
      assert.ok(threeWebxr.capabilities.includes('three-render-host'));
      assert.ok(threeWebxr.capabilities.includes('three-render-host-diagnostics'));
      assert.ok(threeWebxr.capabilities.includes('three-scene-decoration'));
      assert.ok(threeWebxr.capabilities.includes('three-camera-resize'));
      assert.ok(threeWebxr.capabilities.includes('three-controller-select-events'));
      assert.ok(threeWebxr.capabilities.includes('three-animation-loop'));
      assert.ok(threeWebxr.capabilities.includes('three-session-diagnostics'));
    });
    ...
    it('exposes themes with token data', () => {
      assert.equal(data.manifest.themes.length, 1);
      assert.equal(data.manifest.themes[0].name, 'default-provider');
    ...
    it('exposes tokenFiles', () => {
      assert.equal(data.manifest.tokenFiles.length, 2);
    ...
    it('exposes theme rule blocks for cascading agent-built themes', () => {
      ...
      assert.equal(componentAlias.theme, 'default-provider');
    ...
    it('exposes theme recipes for agent composition', () => {
      assert.ok(Array.isArray(data.manifest.themeControls['default-provider']));
      assert.ok(data.manifest.themeControls['default-provider'].some((control) => control.name === 'hue'));
      assert.ok(data.manifest.themeControls['default-provider'].some((control) => control.cssVar === '--sn-theme-density'));
      ...
      let recipe = data.manifest.themeRecipes.find((item) => item.name === 'default-provider');
      assert.ok(recipe, 'default-provider recipe must be discoverable');
