...
function buildTestEditor() {
  ...
  const trigger = new Node('Job Event: RU', {
    id: 'trigger',
  ...
  const fmt1 = new Node('Format: New', {
    id: 'fmt_created',
  ...
  const fmt2 = new Node('Format: Deferred', {
    id: 'fmt_deferred',
    type: 'telegram/format',
    category: 'data',
  });
  ...
  editor.addFrame(
    new Frame('Formatters', { color: '#5cd87a', x: 400, y: 0, width: 300, height: 400 })
  );
  editor.addFrame(
    new Frame('Delivery', { color: '#f0b840', x: 750, y: 100, width: 300, height: 200 })
  );
